...
const PushNotificationContainer = ({dashBoardActiveButton,makeButtonActive}:{dashBoardActiveButton:DashboardButtonState,makeButtonActive:(buttonType:string)=>void}) => {
    return (
        <div className={`${(dashBoardActiveButton.pushNotification.isActive) && ' border-navBarActiveColor  text-navBarActiveColor ' }  bg-gray-400 bg-opacity-20  p-2  flex gap-6  border-s-4  rounded-sm`} onClick={()=>{
